...
router.post("/", authRequired, async (req, res) => {
  ...
  const conflicts = await db("bookings")
    .where({ resource })
    .andWhere(function () {
      this.whereBetween("start_at", [
        s.toISOString(),
        e.toISOString(),
      ]).orWhereBetween("end_at", [s.toISOString(), e.toISOString()]);
    });
  if (conflicts.length > 0)
    return res
      .status(409)
      .json({ error: "Resource already booked for that time" });
  const booking = {
    id: uuidv4(),
    resource,
